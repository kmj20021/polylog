#!/bin/bash
# polylog — /menu 라우트(POST)를 fn-menu 람다에 연결하는 1회용 셋업.
#
# 왜 필요한가:
#   deploy.sh 는 lambda:TagResource 차단으로 CloudFormation(sam deploy)을 못 써서,
#   람다는 CLI 로 직접 만들고 API Gateway 는 '이미 있는' 스테이지를 재배포만 한다.
#   → 새 경로(/menu)의 리소스·메서드·통합·호출권한은 이렇게 한 번 만들어 줘야 한다.
#
# 사용법(CloudShell, 1회):
#   1) fn-menu 람다가 먼저 배포돼 있어야 함 → bash scripts/deploy.sh 먼저.
#   2) bash scripts/setup-menu-route.sh
...
log "✅ 완료 — 엔드포인트: $BASE"
echo ""
echo "검증(메뉴판 사진 menu.jpg 준비 후):"
echo "  B64=\$(base64 -w0 menu.jpg)"
echo "  curl -s -X POST \"$BASE\" -H 'Content-Type: application/json' \\"
echo "    -d \"{\\\"trip_id\\\":\\\"demo-trip\\\",\\\"image_base64\\\":\\\"\$B64\\\",\\\"language\\\":\\\"ko\\\"}\" | python3 -m json.tool"
