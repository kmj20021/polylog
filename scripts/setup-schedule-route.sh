#!/bin/bash
# polylog — /schedule 라우트(POST·GET)를 fn-schedule 람다에 연결하는 1회용 셋업.
#
# 왜 필요한가:
#   deploy.sh 는 lambda:TagResource 차단으로 CloudFormation(sam deploy)을 못 써서,
#   람다는 CLI 로 직접 만들고 API Gateway 는 '이미 있는' 스테이지를 재배포만 한다.
#   → 새 경로(/schedule)의 리소스·메서드·통합·호출권한은 이렇게 한 번 만들어 줘야 한다.
#
# 사용법(CloudShell, 1회):
#   1) fn-schedule 람다가 먼저 배포돼 있어야 함 → bash scripts/deploy.sh 먼저.
#   2) bash scripts/setup-schedule-route.sh
...
echo ""
echo "검증:"
echo "  curl -s \"$BASE?trip_id=demo-trip\"   # GET 타임라인(빈 목록이어야 정상)"
