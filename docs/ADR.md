...
| 2026-06-06 | 2.0.5 | ADR-017 신설 — 대화형 플래너를 `fn-planner`로 분리(Lambda 6종→7종). ADR-001 함수 목록 갱신. |
| 2026-06-07 | 2.0.6 | ADR-016 신설 — 메뉴·영수증 OCR을 Textract→**Bedrock(Claude Haiku) 비전**으로 전환(CJK 인식 한계 해결). 결과적으로 **AWS AI 실사용 = Bedrock 단독**, Textract·Translate 철회. ADR-005·ADR-001·ADR-004·ADR-008 갱신. |
| 2026-06-07 | 2.0.7 | ADR-007 검증 방식을 RS256 로컬 JWKS → Google **tokeninfo** 엔드포인트로 단일화(의존성 0). |
| 2026-06-13 | 2.0.8 | ADR-018 신설 — **메뉴 인앱 분석 폐지, 「구글 렌즈」 단일화**(2026-06-07 코드 결정의 사후 기록). ADR-016의 메뉴 부분(Bedrock 비전 1콜)을 같은 날 오후 렌즈 위임으로 대체. `fn-menu`/`POST /menu`는 배포돼 있으나 앱이 호출하지 않는 **미사용** 상태로 표기. ADR-001·ADR-016 갱신. |
...
| ADR-005 | AWS AI 서비스 3종 "깊이 우선" 전략 (ADR-016으로 부분 대체 — Bedrock 단독) | 대체 | AI 처리 계층 |
...
| ADR-014 | 일정 테이블 단일화 (schedules + schedule-items 통합) | 승인 | 데이터 계층 |
| ADR-015 | 도메인 4종 테이블 trip_id 파티션 통일 | 승인 | 데이터 계층 |
| ADR-016 | 메뉴·영수증 OCR을 Bedrock 비전으로 전환 (Textract·Translate 철회) | 승인 (메뉴 부분 ADR-018로 대체) | AI 처리 계층 |
| ADR-017 | 대화형 플래너 `fn-planner` 분리 (Lambda 6종→7종) | 승인 | 백엔드 |
| ADR-018 | 메뉴 인앱 분석 폐지, 구글 렌즈 단일화 (`fn-menu` 미사용) | 승인 | 메뉴 기능·클라이언트 |
...
| **상태** | 승인 |
| **일자** | 2026-05-25 |
| **영향 범위** | 백엔드 전체 (7개 Lambda 함수, API Gateway REST API) |
...
- fn-menu: ~~메뉴판 사진 번역 + 추천 (Bedrock 비전 1콜, ADR-016)~~ → **미사용**(앱이 메뉴 번역을 구글 렌즈로 위임, ADR-018). 핸들러·`/menu` 라우트는 배포만 잔존.
- fn-receipt: 영수증 OCR + 가계부 (Bedrock 비전 + 환율 API, ADR-016)
- fn-schedule: 일정 CRUD + 여행 CRUD (Google Places + Bedrock)
- fn-planner: 대화형 일정 플래너 (Google Places + Bedrock Haiku·Sonnet, fn-schedule에서 분리 — ADR-017)
- fn-authorizer: API Gateway Lambda Authorizer — Google OAuth ID 토큰을 Google tokeninfo로 검증 (무상태, ADR-007 2026-06-07 갱신 / Kakao 보류)
...
- 트래픽 0일 때 비용도 0
- v1.0 대비 Lambda 11종 → 7종(핵심 6 + 인가 1)으로 간소화, 관리 복잡도 대폭 감소 (2026-06-06 fn-planner 분리로 6종→7종, ADR-017)
...
### 갱신 (2026-06-01) — 타깃 Android 전용으로 축소

> 원안의 "Android + iOS 동시 지원"은 4주 PoC·1인 개발(CON-6) 범위에서 과도하다고 판단해 **Android 전용**으로 축소했다(ADR-007 2026-06-01 갱신과 동기화). `flutter create --platforms=android`, OAuth 클라이언트도 Android 1종만 발급. iOS 빌드는 보류.
>
> - 영향: "iOS 동시 데모" 임팩트는 포기하되, 에뮬레이터 단일 환경으로 E2E 관통 검증을 단순화.
> - `kakao_flutter_sdk` 의존성 제거 — 클라이언트는 `google_sign_in`만 사용(ADR-007 Google 단독 확정).
...
### 갱신 (2026-06-07) — Bedrock 역할 확대(OCR) 및 모델 구성

> ADR-016에 따라 Bedrock(Claude Haiku) **비전**이 영수증의 **OCR까지** 담당하게 되어, Bedrock의 활용 패턴 표에 "이미지 직접 판독(영수증)"이 추가됨. (메뉴 OCR도 ADR-016 시점엔 Bedrock 비전이었으나, 같은 날 오후 품질 문제로 **구글 렌즈에 위임**되어 빠짐 — ADR-018.) 또한 `fn-planner`의 동선 큐레이션에는 품질을 위해 **Sonnet**을 함께 사용(두뇌=Haiku, 큐레이터=Sonnet, ADR-017). "Haiku 위주" 기조는 유지.
...
| 항목 | 내용 |
|---|---|
| **상태** | 대체 (2026-06-07 — ADR-016으로 Textract·Translate 철회, Bedrock 단독) |
| **일자** | 2026-05-25 (원안) / 2026-06-07 (갱신) |
| **영향 범위** | AI 처리 계층 전체 |
...
### 갱신 (2026-06-07) — 3종 → Bedrock 단독 (Textract·Translate 철회, ADR-016)

> 실제 구현에서 Textract(DetectDocumentText)가 한글·일본어(CJK) 메뉴판·영수증을 읽지 못해 비라틴 케이스가 통째로 실패했다. 이를 Bedrock(Claude Haiku) **비전**으로 대체하니 OCR·번역·추천을 한 콜로 처리할 수 있어, Textract와 Translate를 **모두 철회**했다. 자세한 사유·근거는 **ADR-016** 참조.
>
> 결과적으로 "깊이 우선" 전략의 귀결은 **Bedrock 단독, 단일 서비스를 5개 함수에서 다양한 패턴(텍스트 추론 + 비전 + 대화)으로 활용**하는 형태로 더욱 압축됐다. 본 ADR의 "3종" 기술은 이력으로 보존하되, 현행 사실은 ADR-016이 정본이다.
...
## ADR-007: 소셜 OAuth(Google 단독) 인증 (Cognito 대체)
...
**S3에 미디어를 용도별 프리픽스(menus/, receipts/)로 분리 저장하고, 사진 조회에는 CloudFront CDN을 적용한다.**

| 프리픽스 | 용도 | 관련 기능 |
|---|---|---|
| menus/ | 메뉴판 사진 | 서브1 (원안 `photos/` → 구현 시 `menus/`로 명명) |
...
- v2.0에서는 Lambda 7종(핵심 6 + 인가 1) + API Gateway + DynamoDB만 관리 → SAM 축약 문법으로 충분
...
| **상태** | 승인 |
| **일자** | 2026-05-27 |
| **영향 범위** | 모든 Lambda 함수(7종)의 실행 역할, 배포 |
| **관련 요구사항** | NFR-S2 |
...
`SafeRole-polylog`에는 Bedrock / Textract / Translate / DynamoDB(`polylog*`) / S3(`polylog*`) / CloudWatch Logs 권한이 포함되어 있다. (`fn-authorizer`는 외부 Google tokeninfo 조회·토큰 검증만 하므로 이 역할의 권한 중 별도로 요구하는 것은 없다 — ADR-007 2026-06-07 갱신.)
...
## ADR-016: 메뉴·영수증 OCR을 Bedrock 비전으로 전환 (Textract·Translate 철회)

| 항목 | 내용 |
|---|---|
| **상태** | 승인 |
| **일자** | 2026-06-07 |
| **영향 범위** | AI 처리 계층 (`fn-menu`, `fn-receipt`), ADR-005 대체 |
| **관련 요구사항** | TR-AI1~TR-AI3, FR-S1, FR-S2 |

### 맥락 (Context)

ADR-005는 메뉴판 OCR에 **Textract DetectDocumentText**, 번역에 **Translate**, 영수증 OCR에 **Textract AnalyzeExpense**를 쓰는 "AI 3종 깊이 우선" 전략이었다. 그러나 실제 구현·검증 단계에서 **Textract의 DetectDocumentText/AnalyzeExpense가 한글·일본어 등 CJK 문자를 읽지 못한다**는 한계에 부딪혔다(라틴 문자 EN/ES/FR/DE/IT/PT 전용). 여행 앱의 핵심 시나리오인 일본 메뉴판·한국 영수증이 통째로 실패했다.

### 결정 (Decision)

**메뉴·영수증의 OCR을 Textract에서 Amazon Bedrock(Claude 3 Haiku) 비전으로 전환한다. Translate도 철회하고, 번역은 Bedrock이 OCR과 한 콜로 함께 처리한다.**

- `fn-menu`: 사진 → Bedrock 비전이 **판독·번역·한 줄 설명·식이제한 반영 추천을 1콜**로 수행. CJK처럼 비전 번역 품질이 낮은 언어는 `unsupported_language` 신호를 반환해 앱이 "구글 렌즈로 검색" 안내로 유도.
- `fn-receipt`: 사진 → Bedrock 비전이 **금액·항목·통화·날짜를 직접 구조화 추출**(Textract AnalyzeExpense 대체). 이후 환율 API 환산 + Bedrock 카테고리 분류는 유지.
- 결과적으로 **실사용 AWS AI = Bedrock 단독**. Textract·Translate 클라이언트는 코드에서 제거됨.

### 근거 (Rationale)

| 대안 | 장점 | 단점 | 채택 여부 |
|---|---|---|---|
| Textract 유지(ADR-005 원안) | OCR 전용·정확, 활용 서비스 수 다양 | **CJK 미지원**으로 핵심 시나리오 실패, OCR→번역→추천 다단 파이프라인 | 기각 |
| Textract(라틴) + Bedrock 비전(CJK) 혼용 | 각 강점 활용 | 두 경로 분기·유지보수 2배, 라틴도 결국 Bedrock가 더 단순 | 기각 |
| **Bedrock 비전 단독 (채택)** | 모든 언어+통화기호 인식, OCR·번역·추천 1콜, 의존성 0(boto3 내장), 파이프라인 단축 | 단일 서비스 의존 심화, 비전 토큰 비용 | — |

### 결과 (Consequences)

**긍정적**
- CJK 포함 모든 언어 메뉴판·영수증 처리 가능 — 실패 케이스 해소.
- 다단 파이프라인(OCR→Translate→Bedrock)이 **1콜**로 단축 → 지연·코드·실패지점 감소.
- "하나의 서비스(Bedrock)를 텍스트 추론 + 비전 + 대화까지" 더 깊이 활용 — ADR-005 "깊이 우선" 정신의 극단적 귀결.

**부정적**
- AWS AI **커버리지 수치 감소(3종 → 1종)** — 발표 시 "왜 Textract를 버렸나"를 시행착오 사례로 설명(ADR 기반 Q&A).
- Bedrock 의존도 심화 — 장애 시 OCR 기능 전체 영향. PoC 범위에서 수용.
- 비전 입력 이미지 토큰 비용(상한 5MB로 보호).

> ADR-005를 부분 대체. 관련 코드: `backend/src/handlers/menu/app.py`, `backend/src/handlers/receipt/app.py`(둘 다 `boto3.client("bedrock-runtime")` 단일 AI 클라이언트).

### 갱신 (2026-06-07 오후) — 메뉴 부분은 같은 날 「구글 렌즈」로 재전환 (ADR-018)

> 위 결정으로 `fn-menu`에 Bedrock 비전 1콜을 적용한 직후, **같은 날 오후 검증에서 작은 비전 모델 한 호출에 판독·번역·추천을 모두 몰자 품질이 들쭉날쭉**하고 환경 제약상 개선이 어려워, 메뉴 번역은 **실시간 카메라 번역에 특화된 구글 렌즈로 위임**하기로 다시 결정했다(ADR-018). 따라서 **본 ADR의 메뉴 부분(`fn-menu` Bedrock 비전)은 ADR-018로 대체**되며, `fn-menu`/`POST /menu`는 배포돼 있으나 앱이 호출하지 않는 미사용 상태다. **영수증 부분(`fn-receipt` Bedrock 비전)은 정형 데이터라 그대로 유효**하다. 즉 본 ADR의 영수증 결정만 현행이다.

---

## ADR-017: 대화형 플래너 `fn-planner` 분리 (Lambda 6종→7종)

| 항목 | 내용 |
|---|---|
| **상태** | 승인 |
| **일자** | 2026-06-06 |
| **영향 범위** | 백엔드 (`fn-schedule` → `fn-planner` 분리), API Gateway 라우트, ADR-001 함수 목록 |
| **관련 요구사항** | FR-S3.1~S3.5, NFR-P4 |

### 맥락 (Context)

`fn-schedule`이 ① 일정 CRUD ② 대화형 플래너 ③ 순서 재정렬 ④ 여행 CRUD를 모두 담당했는데, ②플래너만 성격이 달랐다. 플래너는 **Bedrock(Haiku+Sonnet) + Google Places**를 호출하는 **무거운(최대 ~30초)** 작업이라, 가벼운 CRUD와 한 함수에 묶이면 (a) 가벼운 호출도 30초 타임아웃·Bedrock/Places 권한을 떠안고, (b) 플래너 버그가 일정·여행 조회까지 함께 죽인다(blast radius 확대).

### 결정 (Decision)

**대화형 플래너(②)만 `fn-planner`로 분리하고 전용 라우트 `POST /planner`를 둔다.** 자원·고장·권한을 격리한다. 저장(담기)은 여전히 `fn-schedule`이 담당하고, `fn-planner`는 읽기+편집(제안)만 한다.

- `fn-planner` 흐름: 이전 대화(`polylog-chats`)+현재 일정(`polylog-schedules`) 로드 → Bedrock #1(두뇌, **Haiku**)이 판단 → 즉시 편집 적용 → Google Places 후보 수집 → Bedrock #2(큐레이터, **Sonnet**)가 방문 순서 동선 제안 → 대화 저장.
- 코드 공유 제약: Places/Bedrock 부품이 `fn-recommend`·`fn-schedule`에도 있으나 **다른 배포 패키지라 import 불가** → 필요한 최소분만 복제(의존성 0·배포 위험 0, 이 코드베이스의 확립된 관례).

### 근거 (Rationale)

- **타임아웃 격리**: 30초짜리 플래너와 100ms짜리 CRUD를 분리해야 각자 적정 타임아웃·메모리 설정 가능.
- **고장 격리**: 가장 복잡한(LLM 2콜+외부 API) 기능의 실패가 단순 조회까지 전파되지 않게.
- **권한 격리**: CRUD 함수는 Places/Bedrock 권한이 불필요(공용 역할이라 실권한은 같으나 책임 경계 명확화).

### 결과 (Consequences)

**긍정적**
- Lambda 단위 책임 명확화 — 함수 = 한 가지 일. blast radius 축소.
- 플래너의 무거운 처리(Sonnet 큐레이션)가 일정 타임라인 응답(NFR-P4)에 영향 없음.

**부정적**
- Lambda 6종 → **7종**(ADR-001 갱신) — 관리 함수 1개 증가.
- Places/Bedrock 헬퍼 코드가 함수 간 복제됨(공유 레이어 미사용 — 의존성 0 원칙의 트레이드오프).

> 관련 코드: `backend/src/handlers/planner/app.py`, `backend/src/handlers/schedule/app.py`. ADR-001 함수 목록·개수 갱신.

---

## ADR-018: 메뉴 인앱 분석 폐지, 구글 렌즈 단일화 (`fn-menu` 미사용)

| 항목 | 내용 |
|---|---|
| **상태** | 승인 |
| **일자** | 2026-06-07 (ADR-016과 같은 날 오후) |
| **영향 범위** | 메뉴 기능, 클라이언트(`menu_screen.dart`, `shared/google_lens.dart`), `fn-menu`/`POST /menu` 비활성, ADR-016 메뉴 부분 대체 |
| **관련 요구사항** | FR-S1.1~S1.4 |

### 맥락 (Context)

ADR-016으로 `fn-menu`가 사진을 **Bedrock(Claude 3 Haiku) 비전**으로 읽어 **판독·번역·한 줄 설명·식이제한 추천을 1콜**로 처리하도록 만들었다. 그러나 같은 날 오후 실제 검증에서 세 가지 문제가 드러났다.
1. **품질 불안정**: 작은 비전 모델 한 호출에 OCR·번역·추천을 모두 몰아넣으니 결과 품질이 메뉴판마다 들쭉날쭉했다(특히 글자 밀도가 높거나 사진 품질이 낮을 때).
2. **알레르기 추정 불가**: 식이제한 반영 추천은 메뉴판 사진에 그 정보가 없어 원리상 '추측'일 수밖에 없다.
3. **환경 제약**: 공용 계정의 권한·배포 제약(ADR-012/013)으로 더 나은 전용 OCR/번역 도구를 붙이기 어렵다.

반면 **구글 렌즈**는 실시간 카메라 번역에 특화돼 라틴·CJK를 가리지 않고 모든 언어를 안정적으로 처리한다.

### 결정 (Decision)

**메뉴 번역을 앱이 직접 수행하지 않고, 「구글 렌즈」 앱으로 위임한다. 인앱 비전 분석(카메라 촬영·라틴/비라틴 분기·`POST /menu` 호출)을 모두 제거하고 렌즈 호출로 단일화한다.**

- `menu_screen.dart`: 안내 패널 + **「구글 렌즈 열기」 버튼**만 둔다. 사진 분석 UI·백엔드 호출 없음.
- `shared/google_lens.dart`: 네이티브(`MainActivity`)의 `MethodChannel('polylog/lens')`를 호출해 **선택창 없이** 설치된 렌즈를 곧장 연다. 미설치 시 `market://`(없으면 https) 플레이스토어 'Google Lens' 설치 페이지로 폴백한다(Android 전용, ADR-003).
- `fn-menu`/`POST /menu`는 **삭제하지 않고 배포 상태로 보존**하되 앱이 호출하지 않는 **미사용(deprecated)** 으로 둔다(롤백 여지·배포 위험 0).

### 근거 (Rationale)

| 대안 | 장점 | 단점 | 채택 여부 |
|---|---|---|---|
| Bedrock 비전 인앱 분석 유지(ADR-016) | 앱 내 완결, AWS AI 활용 어필 | 품질 불안정, 알레르기 추측, 개선 도구 제약 | 기각 |
| 전용 OCR/번역 SDK 추가 도입 | 품질↑ | 환경 제약(권한·배포)·비용·일정 부담, 결국 렌즈만 못함 | 기각 |
| **구글 렌즈 위임 (채택)** | 전 언어 안정·실시간, 앱 코드·백엔드 의존 축소, 사용자에게 더 나은 UX | 외부 앱 의존(설치 필요), 'AWS AI 활용' 어필 1종 감소 | — |

### 결과 (Consequences)

**긍정적**
- 메뉴 번역 품질이 사용자에게 가장 좋은 도구로 안정화 — 라틴/CJK 무관.
- 앱에서 카메라·이미지 인코딩·`/menu` 호출·언어 분기 코드 제거 → 메뉴 화면 단순화·유지보수 부담↓.
- "직접 만들기 vs 검증된 도구 위임"의 의사결정 사례 — 발표 시행착오 근거.

**부정적**
- 외부 앱(구글 렌즈) 설치 의존(미설치 시 스토어 유도로 완화).
- `fn-menu`가 **죽은 엔드포인트**로 남음 — 실제 활성 백엔드는 6종(fn-health·recommend·receipt·schedule·planner·authorizer), 배포는 7종.
- 'AWS AI 다양성' 관점에서 메뉴가 빠지나, 영수증·추천·플래너의 Bedrock 활용은 유지.

> ADR-016의 메뉴 부분을 대체. 관련 코드: `app/lib/features/menu/menu_screen.dart`, `app/lib/shared/google_lens.dart`, `app/android/.../MainActivity`(채널 `polylog/lens`). 영수증(`fn-receipt` Bedrock 비전)은 영향 없음.
