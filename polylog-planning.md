...
- **PoC 범위**: Flutter 크로스 플랫폼 개발 (Android + iOS 동시 지원)
...
| **카메라** | camera / image_picker 패키지 |
| **위치** | geolocator 패키지 |
| **네트워크** | dio |
| **로컬 DB** | sqflite (오프라인 큐) |
| **인증** | AWS Amplify Flutter SDK |
...
1. **클라이언트 계층 (Flutter 앱)**: 카메라·마이크·GPS·텍스트 입력을 수집하고 결과를 표시
...
| 단계 | 주차 | 핵심 마일스톤 |
|---|---|---|
| 1단계: 기반 환경 구축 | 1~2주 | AWS 계정·IAM·Cognito·S3·DynamoDB 구성, Flutter 앱 스켈레톤 |
...
- DynamoDB 테이블 7종 생성
- Flutter 프로젝트 초기 세팅 (Dart + camera + geolocator)

#### 2주차 — 인증 및 앱 스켈레톤
- Cognito User Pool 구성
- Amplify Flutter SDK 통합
- 회원가입·로그인 화면 구현
...
- **Flutter 크로스 플랫폼**: Dart + Flutter Widgets + camera
